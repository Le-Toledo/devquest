...
Se o projeto ainda estiver em `~/Desktop/devquest-main`, mova e reinicie os caches:

```bash
mkdir -p ~/Projects
mv ~/Desktop/devquest-main ~/Projects/
cd ~/Projects/devquest-main
...
Se o projeto ja estiver em `~/Projects/devquest-main`, rode:
...
Alternativa temporaria sem Watchman, util quando o Watchman ficar travado em `Waiting for Watchman watch-project`:

```bash
EXPO_NO_WATCHMAN=1 npx expo start -c
```

Depois que o Metro Bundler abrir, escaneie o QR Code com o Expo Go. Este projeto usa Expo SDK 54 e deve permanecer compativel com Expo Go; ao instalar dependencias Expo, prefira `npx expo install <pacote>` para manter versoes alinhadas ao SDK.
